        ...
        header =
         ...
         <div class="head-extra author-extra">
            <h2 class="contributes">
              contributes to <span>{ if(count==0) "no" else <span class="num">{ count }</span> }</span> { if(count==0) "library" else "libraries" } on <a target="_blank" href={ "https://github.com/%s/" format user }>Github</a> including
            </h2>
          </div>,
        content = <div>
